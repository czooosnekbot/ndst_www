...
<main class="h-100 w-100 d-flex align-items-center flex-column">
                    ...
                    <span class="badge badge-info ml-1">Klasa I</span>
                        {#each data as subject}
                            {#if subject.id.slice(-1) === '1'}
                                <span on:click={filterMaterialsBySubject} class="cursor-pointer badge badge-secondary ml-1 m-1" id="{subject.id}">{subject.fullname}</span>
                            {/if}
                        {/each}
                    {/if}
                </div>
    ...
    <div class="materials d-flex align-items-center flex-column">
        {#if data}
            {#each data as subject}
                    {#each subject.materials as material}
                        <article class="{subject.id} material m-1 d-flex flex-column p-1" style="" id="{material.filename}">
                            ...
                            <div>
                                ...
                                {/if}
                                <span class="d-none" id="{material.filename}--title">{material.title}</span>
                                <span
                                    on:click={getPreview}
